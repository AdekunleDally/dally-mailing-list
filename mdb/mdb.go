...
func UpdateEmail(db *sql.DB, entry EmailEntry) error {
  ...
  opt_out=EXCLUDED.opt_out`, entry.Email, t, entry.OptOut)
...
// 	if err != nil {
// 		//Error to show the table already exist
// 		// if sql.code ==1, this means the table exist
// 		//otherwise, something else(fatal error) occurred
// 		if sqlError, ok := err.(sqlite3.Error); ok {
// 			if sqlError.Code != 1 {
// 				log.Fatal(sqlError)
// 			}
...
// // A function that creates an email entry structure from a database row
// func emailEntryFromRow(row *sql.Rows) (*EmailEntry, error) {
// 	var id int64
// 	var email string
// 	var confirmedAt int64
...
// 		return nil, err
// 	}
// 	// we scan from the db and then convert to a struct
// 	t := time.Unix(confirmedAt, 0) // We are converting the time to an appropriate time structure i.e. a unix time
// 	return &EmailEntry{Id: id, Email: email, ConfirmedAt: &t, OptOut: optOut}, nil
// }
